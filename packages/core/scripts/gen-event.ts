...
import { MatchList, MatchListSchema, PhaseLeaderboard } from '../lib/api/types'
import { ELIMINATION_SCHEDULE, MAX_SCORE_PER_SEED } from '../lib/core/config'
import type { Match } from '../lib/api/types'
import type { BracketEntry, Event } from '../lib/api/types'
import { applyElimination, toSimPlayer, EMPTY_PLAYER } from '../lib/core/simulation'
import { fetchEventMatches, fetchPhaseLeaderboard } from '../lib/api/fetch'
import { existsSync } from 'node:fs'
import { EVENTS } from '../config/events'

const AUTOQUAL_COUNT = 12
...
export function buildEvent(seedMatches: Match[], bonusMap: Map<string, number>): Event {
  ...
  // actively playing (shrinks with cuts AND voluntary drops)
  const active = new Set(field)
  // cut-eliminated only (shrinks with cuts only, not drops)
  ...
  for (let s = 0; s < sorted.length; s++) {
    ...
    // update points and history — only for active players
    for (const uuid of field) {
      ...
      if (completion) {
        ...
        history.push(null) // DNF or voluntary drop
      }
    }

    // rank all non-cut-eliminated players (includes voluntary drops)
    ...
    // apply cut if this is an elimination seed
    ...
    // carry forward ranks of cut-eliminated players, overwrite rest
...
export function getAutoqualifiers(leaderboard: PhaseLeaderboard): Set<string> {
  return new Set(
    leaderboard.users
      .sort((a, b) => b.predPhasePoint - a.predPhasePoint)
      .slice(0, AUTOQUAL_COUNT)
      .map((u) => u.uuid),
  )
}

function sortLeaderboard(users: PhaseLeaderboard['users']) {
...
export function getLCQField(leaderboard: PhaseLeaderboard): string[] {
  const autoquals = getAutoqualifiers(leaderboard)
  const sorted = sortLeaderboard(leaderboard.users)

  const cutoffPlayer = sorted[99]
  const cutoffPred = cutoffPlayer.predPhasePoint
  const cutoffEloRank = cutoffPlayer.eloRank ?? Infinity

  return sorted
    .filter(
      (u) =>
        !autoquals.has(u.uuid) &&
        (u.predPhasePoint > cutoffPred ||
          (u.predPhasePoint === cutoffPred && (u.eloRank ?? Infinity) <= cutoffEloRank)),
    )
    .map((u) => u.uuid)
}

export async function computeBonusMap(
  ...
  const sorted = sortLeaderboard(leaderboard.users).filter((u) => field.has(u.uuid))
...
async function main() {
  ...
  const bonusMap = await computeBonusMap(seedMatches, leaderboard)
  await writeFile(
